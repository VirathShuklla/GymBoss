...
from deps import (
    db, now_utc, today_ist, new_id, get_org_user, audit,
    add_duration, member_status, days_left, member_public,
    next_receipt_no, next_member_code, normalize_phone,
)
...
@router.post("/plans")
async def create_plan(body: PlanBody, user: dict = Depends(get_org_user)):
...
async def update_plan(plan_id: str, body: PlanBody, user: dict = Depends(get_org_user)):
...
async def delete_plan(plan_id: str, user: dict = Depends(get_org_user)):
    res = await db.plans.update_one(
        {"id": plan_id, "organisation_id": user["organisation_id"]},
...
async def create_membership(user: dict, body: MemberBody, enquiry_id: str | None = None) -> dict:
    ...
    due = max(0, round(price - body.discount - body.amount_paid, 2))
    ts = now_utc()
...
@router.post("/members")
async def create_member(body: MemberBody, enquiry_id: Optional[str] = None, user: dict = Depends(get_org_user)):
...
async def update_member(member_id: str, body: MemberUpdateBody, user: dict = Depends(get_org_user)):
...
async def delete_member(member_id: str, user: dict = Depends(get_org_user)):
...
async def freeze_member(member_id: str, body: FreezeBody, user: dict = Depends(get_org_user)):
    m = await db.members.find_one({"id": member_id, "organisation_id": user["organisation_id"], "deleted_at": None})
    if not m:
        raise HTTPException(404, "Member not found")
...
async def unfreeze_member(member_id: str, user: dict = Depends(get_org_user)):
    res = await db.members.update_one(
        {"id": member_id, "organisation_id": user["organisation_id"], "deleted_at": None},
...
async def renew_member(member_id: str, body: RenewBody, user: dict = Depends(get_org_user)):
    org_id = user["organisation_id"]
    ...
    await db.members.update_one({"id": member_id}, {"$set": {
        ...
        "due_amount": due, "discount": body.discount, "frozen_until": None, "updated_at": ts,
    }})
    if body.amount_paid > 0:
        await db.payments.insert_one({
...
async def record_payment(body: PaymentBody, user: dict = Depends(get_org_user)):
    org_id = user["organisation_id"]
    ...
    await db.members.update_one({"id": m["id"]}, {"$set": {"due_amount": new_due}})
    await db.organisations.update_one({"id": org_id}, {"$set": {"onboarding.payment": True}})
