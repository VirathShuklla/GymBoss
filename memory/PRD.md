...
**M4 — Communications**: Individual WhatsApp templates (welcome/payment reminder/expiring/expired/birthday/follow-up/custom), payment reminders, receipts/invoices (view + print). Bulk Announcements module REMOVED per user decision; raise-support-request form removed (support = WhatsApp/Email cards only).
**M5 — Business + Billing**: Finance (range filters, revenue/expense/net/outstanding, by-method, by-category, transactions), Reports (revenue trend, member growth, outstanding dues, membership expiry, enquiry conversion), Export Center (7 CSV datasets), Subscription page + Razorpay order/verify/webhook flow with super-admin-managed keys (settings override env), dynamic plan price (plan_price_inr in platform settings propagates to landing hero/pricing/FAQ/CTA, paywall, subscription page, billing amount, MRR), BuildVVO Super Admin console (overview KPIs, gyms table, gym detail, disable/reactivate with audit, platform settings, audit logs).
...
- Razorpay: super admin can paste Key ID/Secret/Webhook Secret in /superadmin → Platform Settings (DB settings override backend/.env `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET`/`RAZORPAY_WEBHOOK_SECRET`) to activate online billing
